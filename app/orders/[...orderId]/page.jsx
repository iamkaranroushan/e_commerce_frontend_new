...
const OrderDetailPage = () => {
  ...
  const getStatusIndex = (status) => ORDER_STATUS.indexOf(status);
          ...
          {ORDER_STATUS.map((status, index) => {
            ...
            return (
              <div key={status} className="z-20 flex flex-col items-center flex-1">
                ...
                />
                <span className={`mt-2 text-xs font-medium text-center ${isActive ? "text-green-600" : "text-gray-400"}`}>
                  {status}
                </span>
              </div>
            );
          ...
          {order.orderItems.map((item) => (
            <div key={item.id} className="flex items-start gap-4 border-b pb-4 last:border-b-0">
              ...
              <div className="flex-1 space-y-1">
                <h3 className="text-[16px] lg:text-lg font-semibold text-gray-800">
